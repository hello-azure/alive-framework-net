...
namespace PanGu
{
    public class Segment
    {
        ...
        ///把词源分发给个自己的字典
        /// </summary>
        static private void LoadDictionary(Dictionary<string, List<string>> dic)
        {
                    ...
                    case "PWordSource":
                        {
                            ...
                            _WordDictionary.Load(dic[key]);//词源目录
                            break;
                        }
                    case "PChsSingleName":
                        {
                            _ChsName = new PanGu.Dict.ChsName();//加载中文名称
                            _ChsName.LoadChsSingleName(dic[key]);
                            _WordDictionary.ChineseName = _ChsName;
                            break;
                        }
                    case "PChsDoubleName1":
                        {
                            _ChsName.LoadChsDoubleName1(dic[key]);
                            break;
                        }
                    case "PChsDoubleName2":
                        {
                            _ChsName.LoadChsDoubleName2(dic[key]);
                            break;
                        }
                    case "PStopWord":
                        {
                            _StopWord = new PanGu.Dict.StopWord();
                            _StopWord.LoadStopwordsDict(dic[key]);//加载停用词
                            break;
                        }
                    case "PSynonym":
                        {
                            _Synonym = new PanGu.Dict.Synonym();
                            _Synonym.Load(dic[key]);//是否输出同义词
                            break;
                        }
                    case "PWildcard":
                        {
                            ...
                            _Wildcard.Load(dic[key]);//是否输出通配符
                            break;
                        }
                    default:
                        break;
        ...
        public static void Init()
        {
            lock (_LockObj)
            {
                if (_Inited)//第一次加载词源 后 _Inited为true,
                {
                    return;
                }
            }
        ...
        public static void Init(Dictionary<string, List<string>> dic)
        {
            lock (_LockObj)
            {
                if (_Inited)//第一次加载词源 后 _Inited为true,
