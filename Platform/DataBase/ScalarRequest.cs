...
{
    /// <summary>
    /// 执行一个带查询的数据库操作。并返回查询结果的第一行第一列（作为输出参数ScalarValueName返回）。
    /// </summary>
    public class ScalarRequest : DbRequest
    {
        #region ==== 常数 ====

        /// <summary>
        /// 存储过程返回的查询结果的第一行，第一列的值。
        /// </summary>
        public static readonly string ScalarValueName = "ScalarValueName";
        ...
        protected override DbResult DoExecute()
        {
            ...
            try
            {
                object scelarValue = command.ExecuteScalar();

                if (IsExecuteProcedure)
                {
                    result.OutputParameters = this.ReadOutputParameters(command);
                    result.OutputParameters.Add(ScalarValueName, scelarValue);
            ...
            catch (Exception ex)
            {
                ...
                GlobalLogger<ScalarRequest>.Error(ex.ToString());
            }
