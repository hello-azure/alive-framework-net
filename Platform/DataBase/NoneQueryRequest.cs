...
namespace Alive.Foundation.Storage
{
    ...
    /// 执行一个不返回数据的数据库操作。
    /// </summary>
    public class NoneQueryRequest : DbRequest
    {
        #region ==== 常数 ====
        ...
        /// <summary>
        /// 当前数据库操作没有对任何数据记录产生影响时返回的错误代码。
        /// </summary>
        public static readonly string NoEffectedError = "NoEffectedError";
        ...
        public NoneQueryRequest(string sql)
        {
            this.SQL = sql;
        }
        ...
        protected override DbResult DoExecute()
        {
            ...
            try
            {
                ...
                if (effect == 0)
                {
                    result.ErrorCode = NoEffectedError;
                }

                if (IsExecuteProcedure && effect != 0)
                {
            ...
            catch (Exception ex)
            {
                ...
                GlobalLogger<NoneQueryRequest>.Error(ex.ToString());
            }
